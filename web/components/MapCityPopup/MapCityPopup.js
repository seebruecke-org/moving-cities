...
export default function MapCityPopup({ name, intro, coordinates, slug, country: { slug: countrySlug } }) {
    return <Popup coordinates={convertStrapiToMapbox(coordinates)}>
        <Global styles={styles.resetPopup} />
        <div css={styles.container}>
            <h3 css={styles.title}>{name}</h3>

            {intro && (
                <p css={styles.intro}>{intro}</p>
            )}
