...
export async function fetchFeaturedCities(client, locale) {
  ...
  const citiesWithCoordiantes = cities.data?.map(({ attributes: { coordinates, ...city } }) => ({
...
export async function fetchCounts(client, locale) {
  const counts = await fetchAPI(
    client,
    `
    query Counts($locale: String) {
      citiesCount(locale: $locale)
      featuredCitiesCount(locale: $locale)
      networksCount(locale: $locale)
