...
export async function fetchApproachLocalizationsBySlug(client, locale, slug) {
  ...
  return approach?.localizations?.reduce((a, v) => ({...a, [mapStrapiToFELocale(v.locale)]: `/${mapStrapiToFELocale(v.locale)}/${v.city.slug}/${v.slug}`}), {});
}
...
export async function fetchApproachCategoryLocalizationsBySlug(client, locale, filter) {
  if (filter && filter !== "") {
    ...
    return await approachCategory?.localizations?.reduce(async (a, v) => {
      ...
      await i18n.init();
      return ({...await a, [localizationLocale]: `/${localizationLocale}/${i18n.t('approaches', { ns: 'slugs', lng: localizationLocale })}/${v.slug}`})
    }, {});
  } else {
    const { locales } = i18n;
