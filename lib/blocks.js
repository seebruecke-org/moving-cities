...
import networksSummary, { sideload as sideloadNetworksSummary } from '@/components/Blocks/NetworksSummary/block';
...
export async function sideloadBlockData(blocks, context, locale) {
  return await Promise.all(blocks.map(async ({ __typename, ...data }) => {
